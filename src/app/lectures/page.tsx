...
export const metadata: Metadata = { title: "강의공고" };
...
export default async function LecturesPage({
  ...
  const [lectures, totalCount] = await Promise.all([
    getPostsByBoardPaginated("lecture", page, PER_PAGE, search || undefined),
    getPostCount("lecture"),
  ]);
  const totalPages = Math.ceil(totalCount / PER_PAGE);

  return (
    <>
      <SubpageHero title="강의공고" breadcrumb={[{ label: "강의공고" }]} />
      <section className="py-16 lg:py-20">
        <div className="container-custom max-w-4xl">
          <SearchBar basePath="/lectures" placeholder="강의공고 검색" defaultValue={search} />
                ...
                {lectures.map((post, i) => (
                  <tr key={post.id} className="hover:bg-surface/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/lectures/${post.id}`} className="text-text hover:text-primary transition-colors font-medium">
                        {post.is_pinned && <span className="text-xs text-accent font-semibold mr-1.5">[공고]</span>}
                        {post.title}
                      </Link>
                    </td>
                ...
                {lectures.length === 0 && (
                  <tr>
                    <td colSpan={4} className="text-center py-8 text-sm text-text-muted">
                      {search ? `"${search}" 검색 결과가 없습니다.` : "강의공고가 없습니다."}
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <Pagination currentPage={page} totalPages={totalPages} basePath={search ? `/lectures?q=${search}` : "/lectures"} />
        </div>
      </section>
    </>
  );
